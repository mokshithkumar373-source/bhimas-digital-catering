...
function convertColorToRgb(colorStr: string, originalGetComputedStyle: typeof window.getComputedStyle): string {
  ...
  const computed = originalGetComputedStyle(tempColorEl).color;
...
export async function nodeToCanvas(node: HTMLElement) {
  ...
  // 1. Mock document.styleSheets to bypass html2canvas stylesheet parser
  Object.defineProperty(document, "styleSheets", {
    value: [],
    configurable: true,
  });

  // 2. Proxy getComputedStyle to resolve oklch/oklab to rgb
  ...
  try {
    ...
    // Restore original window helper
